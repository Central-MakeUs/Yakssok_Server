...
class FriendServiceTest {
	...
	class FollowByInviteCode {
		...
		@Nested
		@DisplayName("followFriendByInviteCode - 실패 케이스")
		class FollowByInviteCode_Fail {

			@Test
			@DisplayName("초대코드로 대상 유저 조회 실패시: 예외 전파 & save 호출 안 됨")
			...
			@Test
			@DisplayName("관계 검증에서 막힐 때: 예외 전파 & toFriend/save 호출 안 됨")
		...
		@Test
		@DisplayName("초대코드로 팔로우: 관계검증 후 Friend 저장")
		void followFriendByInviteCode_saves() {
			// given
			Long userId = 1L;
			...
			when(followFriendRequest.toFriend(same(user), same(following))).thenReturn(friend);

			// when
			friendService.followFriendByInviteCode(userId, followFriendRequest);

			// then
			InOrder inOrder = inOrder(userService, relationshipService, friendRepository);
			...
			inOrder.verify(friendRepository).save(same(friend));
